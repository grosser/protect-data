...
class JavaScriptTestTask < ::Rake::TaskLib
  ...
  def browser(browser,path=nil)
    browser =
      case(browser)
        when :firefox
          FirefoxBrowser.new path if path
          FirefoxBrowser.new unless path
        when :safari
          SafariBrowser.new path if path
          SafariBrowser.new unless path
        when :ie
          IEBrowser.new path if path
          IEBrowser.new unless path
        when :konqueror
          KonquerorBrowser.new path if path
          KonquerorBrowser.new unless path
        when :opera
          OperaBrowser.new path if path
          OperaBrowser.new unless path
        else
          browser
      end
